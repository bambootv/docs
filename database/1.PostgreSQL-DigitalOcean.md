...
### Option 2: Read-Only Specific Tables
```sql
...
GRANT SELECT ON public.users TO my_limited_user;
GRANT SELECT ON public.orders TO my_limited_user;
GRANT SELECT ON public.products TO my_limited_user;

GRANT USAGE ON SEQUENCE public.users_id_seq TO my_limited_user;
GRANT USAGE ON SEQUENCE public.orders_id_seq TO my_limited_user;
```
...
### Giải thích
- `-F p`: dump SQL thuần
- `--no-owner`: không set owner
- `--no-privileges`: không export GRANT/REVOKE
- Tránh lỗi permission khi restore

### Fix lỗi permission khi restore
```sql
